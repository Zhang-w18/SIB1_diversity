...
生成配置：`D:\07 2026\Simulation\SIB1_diversity\configs\uma_7ghz_48prb.yaml`  
...
| SSB AE 权重范数最大偏差 | 2.442e-15 |
| Secondary AE 权重范数最大偏差 | 2.665e-15 |
...
$$
A_{sub}(uV) = (1/\sqrt(6)) * \sum_{m=0..5} exp(j*2*pi*m*dV*uV)
$$


第一零点满足 `abs(sin(eta_local)) = 1/(6*dV)`。代入 `dV=0.8` 和12°机械下倾，向下侧第一公共零陷位于全局 η=24.025°，落在 UE 目标范围 11.549°～33.879° 内。

![TXRU subarray vertical factor](txru_subarray_vertical_factor.png)

这是映射矩阵施加在所有 TXRU steering vector 上的公共乘法因子，因此调整 TXRU 数字码本、改用4H×2V布局或增加 weighted-LS 迭代均不能填平该零陷。当前主配置的“SSB共同覆盖目标角域”验收项判定为 **未通过**。
...
SSB 目标区统计：全码本最低区内增益 -20.59 dB，最大区内 ripple 37.28 dB。逐波束精确数值见 `beam_metrics.csv`。
...
Secondary 目标区统计：全码本最低区内增益 -18.99 dB，最大区内 ripple 37.95 dB。
...
- 继续 UMa/BLER 集成前，需要研究者在以下物理配置修订中选择一种：将面板机械下倾移至目标区中部附近；允许6×1子阵内部加入固定电下倾相位；或改变垂直 TXRU/AE 分组。维持12°机械下倾和6×1等幅同相映射会保留目标区内的不可消除零陷。
