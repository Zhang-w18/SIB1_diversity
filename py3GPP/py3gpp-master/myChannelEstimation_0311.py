...
def estimate_symbol_by_blocks_M(
    ...
    return H_col, True
def _normalize_dmrs_cols(dmrs_l, refInd, K, N):
    """
    返回排序后的 DMRS 符号列号（0-based）。
    优先使用 dmrs_l；若 dmrs_l is None，则从 refInd 自动检测。
    """
    cols_from_ref = np.array([], dtype=int)
    if refInd is not None and len(refInd) > 0:
        cols_from_ref = np.unique((np.asarray(refInd).astype(int) // K))
    ...
    if dmrs_l is None:
        ...
        # 若 refInd 中能检测到实际 DMRS 列，则与之取交集，避免传参和 refGrid 不一致
        if cols_from_ref.size > 0:
            inter = cols[np.isin(cols, cols_from_ref)]
            if inter.size > 0:
                cols = inter
            else:
                # 若传入 dmrs_l 和 refGrid 完全对不上，则回退到 refGrid 的实际位置
                cols = cols_from_ref

    return np.array(sorted(set(cols.tolist())), dtype=int)


def _fill_missing_in_freq(H_col):
    """
    对单个符号列中仍为 0 的 RE 做一次频域补洞。
    """
    missing = np.where(H_col == 0)[0]
    if missing.size == 0:
        return H_col

    known = np.where(H_col != 0)[0]
    if known.size == 0:
        return H_col

    if known.size == 1:
        H_col[missing] = H_col[known[0]]
        return H_col

    H_col = H_col.copy()
    H_col.real[missing] = np.interp(missing, known, H_col.real[known])
    H_col.imag[missing] = np.interp(missing, known, H_col.imag[known])
    return H_col


def _time_interp_from_dmrs_cols(H, has_dmrs_col, dmrs_cols=None):
    """
    沿时间轴（symbol 方向）做插值。
    - 若只有 1 个 DMRS 列：复制到其他列
    - 若 >=2 个 DMRS 列：对实部/虚部分别线性插值
    """
    K, N = H.shape

    if dmrs_cols is None:
        cols_with = np.where(has_dmrs_col)[0]
    else:
        dmrs_cols = np.asarray(dmrs_cols, dtype=int)
        dmrs_cols = dmrs_cols[(dmrs_cols >= 0) & (dmrs_cols < N)]
        cols_with = dmrs_cols[has_dmrs_col[dmrs_cols]]

    cols_with = np.array(sorted(set(cols_with.tolist())), dtype=int)
    cols_wo = np.setdiff1d(np.arange(N), cols_with)
...
def _normalize_dmrs_cols(dmrs_l, refInd, K, N):
    """
    返回排序后的 DMRS 符号列号（0-based）。
    若 dmrs_l is None，则从 refInd 自动检测。
    """
    cols_from_ref = np.array([], dtype=int)
    if refInd is not None and len(refInd) > 0:
        cols_from_ref = np.unique((np.asarray(refInd).astype(int) // K))
    ...
    if dmrs_l is None:
        ...
        if cols_from_ref.size > 0:
            inter = cols[np.isin(cols, cols_from_ref)]
            cols = inter if inter.size > 0 else cols_from_ref

    return np.array(sorted(set(cols.tolist())), dtype=int)


def _scalarize_nvar(nVar):
    if np.isscalar(nVar):
        return float(np.real(nVar))
    nVar = np.asarray(nVar)
    if nVar.size == 0:
        return 0.0
    return float(np.mean(np.real(nVar)))


def _time_interp_from_dmrs_cols(H, has_dmrs_col, dmrs_cols=None):
    """
    沿时间轴（symbol 方向）做线性插值：
    - 1 个 DMRS 列：复制
    - 多个 DMRS 列：线性插值
    """
    K, N = H.shape
...
def _freq_corr_exp_pdp(delta_k, scs_hz, tau_rms_s):
    """
    频域相关模型（指数 PDP 对应的一种常见简化形式）
    delta_k : 子载波间隔差（单位：个子载波）
    """
    delta_f = np.asarray(delta_k, dtype=np.float64) * float(scs_hz)
    return 1.0 / (1.0 + 1j * 2.0 * np.pi * float(tau_rms_s) * delta_f)

def mmse_interpolate_bundle_from_ls(
        h_ls_pilots,
        pilot_pos,
        bundle_len,
        nVar,
    ...
    """
    在一个 bundle 内，用 pilot 上的 LS 信道估计，做频域 LMMSE/MMSE 恢复整个 bundle。

    输入:
      h_ls_pilots : shape=(Np,) 该 bundle 内导频位置上的 LS 信道估计
      pilot_pos   : shape=(Np,) pilot 在 bundle 内的相对频域下标，例如 [0,2,4,...]
      bundle_len  : bundle 子载波数，例如 24
      nVar        : AWGN 噪声方差（标量）
      pilot_power : 导频平均功率（QPSK 通常≈1）
      scs_khz     : SCS
      tau_rms_s   : MMSE 相关模型参数
      reg         : 对角加载

    输出:
      h_bundle_hat : shape=(bundle_len,) 该 bundle 内所有子载波的信道估计
    """
    pilot_pos = np.asarray(pilot_pos, dtype=np.int32)
    h_ls_pilots = np.asarray(h_ls_pilots, dtype=np.complex128)

    Np = len(pilot_pos)
    if Np == 0:
        return np.zeros(bundle_len, dtype=np.complex64), False

    if Np == 1:
        # 极端 fallback：只有一个点时直接平坦外推
        return np.full(bundle_len, h_ls_pilots[0], dtype=np.complex64), True

    nVar_eff = _scalarize_nvar(nVar)
    sigma_ls = nVar_eff / max(float(pilot_power), 1e-12)

    scs_hz = float(scs_khz) * 1e3

    # 目标频点：bundle 内所有子载波
    k_all = np.arange(bundle_len, dtype=np.float64)[:, None]    # (L,1)
    k_p = pilot_pos.astype(np.float64)[None, :]                 # (1,Np)
    delta_ap = k_all - k_p                                      # (L,Np)

    # pilot-pilot 相关
    k_p_col = pilot_pos.astype(np.float64)[:, None]             # (Np,1)
    delta_pp = k_p_col - k_p_col.T                              # (Np,Np)

    Rap = _freq_corr_exp_pdp(delta_ap, scs_hz, tau_rms_s)       # (L,Np)
    Rpp = _freq_corr_exp_pdp(delta_pp, scs_hz, tau_rms_s)       # (Np,Np)

    A = Rpp + (sigma_ls + reg) * np.eye(Np, dtype=np.complex128)

    try:
        w = np.linalg.solve(A, h_ls_pilots)
    except np.linalg.LinAlgError:
        w = np.linalg.pinv(A) @ h_ls_pilots

    h_bundle_hat = Rap @ w
    return h_bundle_hat.astype(np.complex64), True
import numpy as np


def _freq_corr_real_even(delta_k, scs_hz, tau_rms_s, model="gaussian"):
    """
    更稳的频域相关模型（实对称）：
      - gaussian: exp(-0.5 * (2*pi*tau*df)^2)
      - cauchy  : 1 / (1 + (2*pi*tau*df)^2)

    返回实数相关系数，避免人为引入相位偏差。
    """
    delta_f = np.asarray(delta_k, dtype=np.float64) * float(scs_hz)
    x = 2.0 * np.pi * float(tau_rms_s) * delta_f
    ...
    if model == "gaussian":
        ...
        raise ValueError("model must be 'gaussian' or 'cauchy'")

def _linear_interp_complex(pilot_pos, h_pilot, L):
    """
    对 bundle 内做复数线性插值（实部/虚部分别插）
    """
    pilot_pos = np.asarray(pilot_pos, dtype=int)
    h_pilot = np.asarray(h_pilot, dtype=np.complex128)

    out = np.zeros(L, dtype=np.complex128)
    ...
    out.real = np.interp(x, pilot_pos, h_pilot.real)
    out.imag = np.interp(x, pilot_pos, h_pilot.imag)
    return out.astype(np.complex64)
def mmse_interpolate_bundle_from_ls_robust(
        h_ls_pilots,
        pilot_pos,
        bundle_len,
        nVar,
    ...
    """
    稳健版 bundle 内 MMSE + 线性插值融合

    输入:
      h_ls_pilots : DMRS 上的 LS 信道估计
      pilot_pos   : DMRS 在 bundle 内的相对位置
      bundle_len  : bundle 长度（子载波数）
      nVar        : 噪声方差
      pilot_power : DMRS 平均功率
    输出:
      h_hat_bundle
    """
    pilot_pos = np.asarray(pilot_pos, dtype=int)
    ...
    if Np == 1:
        h_lin = np.full(bundle_len, h_ls_pilots[0], dtype=np.complex64)
        return h_lin, True

    # ---- 1) 线性插值基线 ----
    h_lin = _linear_interp_complex(pilot_pos, h_ls_pilots, bundle_len).astype(np.complex128)

    # ---- 2) 噪声方差、LS噪声方差 ----
    nVar_eff = float(np.mean(np.real(nVar))) if not np.isscalar(nVar) else float(np.real(nVar))
    sigma_ls = nVar_eff / max(float(pilot_power), 1e-12)

    # ---- 3) 从 LS pilots 自适应估信道功率 ----
    # 避免默认 channel variance = 1 带来的幅度塌缩
    p_ls = float(np.mean(np.abs(h_ls_pilots) ** 2))
    sigma_h2 = max(p_ls - sigma_ls, 1e-4)

    lam = sigma_ls / sigma_h2
    ...
    Cap = _freq_corr_real_even(delta_ap, scs_hz, tau_rms_s, model=corr_model)
    Cpp = _freq_corr_real_even(delta_pp, scs_hz, tau_rms_s, model=corr_model)

    A = Cpp + (lam + reg) * np.eye(Np, dtype=np.float64)
    ...
    # ---- 5) 自适应融合：低SNR更信任线性插值，高SNR更信任MMSE ----
    # alpha 越大，越信任 MMSE
    alpha = sigma_h2 / (sigma_h2 + 2.0 * sigma_ls + 1e-12)
    alpha = float(np.clip(alpha, alpha_min, alpha_max))

    h_hat = alpha * h_mmse + (1.0 - alpha) * h_lin

    # ---- 6) pilot 位置强制锚回 LS ----
    h_hat[pilot_pos] = h_ls_pilots

    return h_hat.astype(np.complex64), True
def estimate_symbol_by_blocks_MMSE_robust(
        col,
        K,
        refInd,
        rxSym_all,
    ...
    对某个 symbol 列:
      - 按 bundle 切块
      - 每个 bundle 内用 robust MMSE + 线性插值 融合恢复整块
    """
    H_col = np.zeros((K,), dtype=np.complex64)
    L_block = rb_size * 12
    ...
    sc_idx = (refInd[in_col] - col * K).astype(int)
    y = np.asarray(rxSym_all[in_col], dtype=np.complex128)
    x = np.asarray(refSym_all[in_col], dtype=np.complex128)

    eps = 1e-12
    x_pow = np.maximum(np.abs(x) ** 2, eps)
    h_ls_all = y * np.conj(x) / x_pow

    num_blocks = (K + L_block - 1) // L_block
    filled_any = False
    ...
    for block_idx in range(num_blocks):
        ...
        h_ls_blk = h_ls_all[in_blk]
        pilot_pow_blk = float(np.mean(x_pow[in_blk]))

        h_blk_hat, ok = mmse_interpolate_bundle_from_ls_robust(
            h_ls_pilots=h_ls_blk,
            pilot_pos=dmrs_rel,
            bundle_len=L_blk,
            nVar=nVar,
            pilot_power=pilot_pow_blk,
            scs_khz=scs_khz,
            tau_rms_s=tau_rms_s,
    ...
    return H_col, filled_any

def myChannelEstimate_bundle_MMSE_robust(
        rxGrid=None,
        refInd=None,
        refSym=None,
        refGrid=None,
    ...
    if rxGrid is None:
        raise ValueError("rxGrid must not be None")

    if refGrid is None:
        if (refInd is None) or (refSym is None):
            raise ValueError("refInd and refSym need to be given when refGrid is None!")
        refGrid = np.zeros(rxGrid.shape, dtype=complex)
        nrSetResources(refInd, refGrid, refSym)
    else:
        pilot_idx = np.where(refGrid.ravel(order="F") != 0)[0]
        refInd = pilot_idx
        refSym = refGrid.ravel(order="F")[refInd]
    ...
    dmrs_cols = _normalize_dmrs_cols(dmrs_l, refInd, K, N)

    for col in dmrs_cols:
        H_col, ok = estimate_symbol_by_blocks_MMSE_robust(
            col=col,
            K=K,
            refInd=refInd,
            rxSym_all=rxSym_all,
    ...
    H = _time_interp_from_dmrs_cols(H, has_dmrs_col, dmrs_cols=dmrs_cols)
    return H
...
def myChannelEstimate_std(
    ...
    # 自动/显式解析 DMRS 所在 symbol 列
    dmrs_cols = _normalize_dmrs_cols(dmrs_l, refInd, K, N)

    # --------------------------------------------------------
    # 关键策略：
    # 对 PDSCH 多 DMRS 场景，使用“逐 DMRS 符号估计 + 时间插值”
    # 这样可以直接复用你现在 estimate_symbol_by_blocks_M / denoise 算法
    # --------------------------------------------------------
    use_per_dmrs_symbol_mode = True

    # 如果你以后还想保留原来 EST_TFDOMAIN=True 且连续符号块联合处理的逻辑，
    # 只有当 DMRS 符号是连续的、且数量与 N_sym 匹配时再启用。
    if EST_TFDOMAIN and dmrs_cols.size == N_sym and dmrs_cols.size >= 2:
        diffs = np.diff(dmrs_cols)
        if np.all(diffs == 1):
            use_per_dmrs_symbol_mode = False

    if use_per_dmrs_symbol_mode:
        # ---------- 每个 DMRS 符号独立做频域估计 ----------
        for col in dmrs_cols:
            if EST_PBCH:
            ...
            if ok:
                H_col = _fill_missing_in_freq(H_col)
                H[:, col] = H_col
                has_dmrs_col[col] = True
    ...
    # --------------------------------------------------------
    # 保留你原来的“连续多符号联合处理”逻辑（比如某些特殊场景）
    # 仅当 DMRS 列连续且数量正好等于 N_sym 时进入
    # --------------------------------------------------------
    L_block = rb_size * 12

    # 这里假设 dmrs_cols 是连续的，例如 [2,3] 这种
    start_sym = int(dmrs_cols[0])
    ...
    for col in dmrs_cols:
        if has_dmrs_col[col]:
            H[:, col] = _fill_missing_in_freq(H[:, col])
...
# def ce_subband_td_denoise_interpolate( Hest_dmrs, # shape = (Ndmrs,) 该子带 DMRS 上的 LS 信道 dmrs_pos, # shape = (Ndmrs,) 该子带内 DMRS 相对索引(0..L-1) L, # 子带长度(如 24) up_factor=None, # 插值倍率；默认 L/Ndmrs（PBCH: 24/6=4） energy_keep=0.9,# 时延域保留能量比例(0~1) guard=0, # 可选：对窗口左右各扩一点tap pilot_consistency=True # 回频域后在导频处回填LS，保证一致性/幅度不变 ): """ 返回: H_sub (L, ) 为该子带所有 L 个频点的估计（复数） """ Hest_dmrs = np.asarray(Hest_dmrs, dtype=np.complex64).ravel() dmrs_pos = np.asarray(dmrs_pos, dtype=int).ravel() Nd = Hest_dmrs.size assert Hest_dmrs.size == dmrs_pos.size and Hest_dmrs.size > 0 L = int(L) # ===== 0) 检查等间隔 & 提取 offset ===== # 期望 dmrs_pos = offset + 4*n diffs = np.diff(np.sort(dmrs_pos)) if not np.all(diffs == diffs[0]): raise ValueError("该函数要求本子带 DMRS 等间隔（PBCH为间隔=4）。") spacing = int(diffs[0]) if Nd > 1 else 4 if spacing <= 0: spacing = 4 offset = int(dmrs_pos.min() % spacing) # 把 DMRS 样点按 n=0..Nd-1 的顺序排好：pos = offset + spacing*n n_idx = ((dmrs_pos - offset) // spacing).astype(int) order = np.argsort(n_idx) H6 = Hest_dmrs[order] n_sorted = n_idx[order] # 保险：确保是 [0,1,2,3,4,5] if not np.array_equal(n_sorted, np.arange(Nd)): raise ValueError("DMRS位置与期望网格不一致（应为 offset+4*n 且 n 连续）。") # ===== 1) 对“6点 DMRS 序列”做单位化 IFFT → 时延域 ===== h6 = _ifft_u(H6) # shape=(6,) import matplotlib.pyplot as plt plt.figure(figsize=(8, 4)) plt.plot(np.abs(h6[:]), label='|h_td| (symbol 0)') plt.xlabel('Sample index (delay)') plt.ylabel('Magnitude') plt.title('Time-domain impulse response magnitude') plt.grid(True) plt.legend() plt.show() # ===== 2) 时延域能量窗：找主要能量并加窗，其余置零（可选连续窗）===== p = np.abs(h6) ** 2 if p.sum() > 0: order_e = np.argsort(p)[::-1] csum = np.cumsum(p[order_e]) cutoff = energy_keep * csum[-1] k_keep = np.searchsorted(csum, cutoff) + 1 peak = order_e[0] left = right = int(peak) e_acc = p[peak] while e_acc < cutoff and (left > 0 or right < Nd - 1): cand_left = p[left - 1] if left > 0 else -1 cand_right = p[right + 1] if right < Nd - 1 else -1 if cand_left >= cand_right and cand_left >= 0: left -= 1; e_acc += p[left] elif cand_right >= 0: right += 1; e_acc += p[right] else: break left = max(0, left - guard) right = min(Nd - 1, right + guard) win6 = np.zeros((Nd,), dtype=np.float32) win6[left:right + 1] = 1.0 else: win6 = np.ones((Nd,), dtype=np.float32) # 全零能量则不裁 h6_win = h6 * win6 plt.figure(figsize=(8, 4)) plt.plot(np.abs(h6_win[:]), label='|h_td| (symbol 0)') plt.xlabel('Sample index (delay)') plt.ylabel('Magnitude') plt.title('Time-domain impulse response magnitude') plt.grid(True) plt.legend() plt.show() # ===== 3) 时延域补零：从 6 点 → 24 点（上采样因子 4），保持单位化能量标度 ===== up = L // Nd if up * Nd != L: raise ValueError("L 必须是 Nd 的整数倍（PBCH: 24 = 6 * 4）") hL = np.zeros((L,), dtype=np.complex64) hL[:Nd] = h6_win # 直接尾部补零（也可换成居中放置） plt.figure(figsize=(8, 4)) plt.plot(np.abs(hL[:]), label='|h_td| (symbol 0)') plt.xlabel('Sample index (delay)') plt.ylabel('Magnitude') plt.title('Time-domain impulse response magnitude') plt.grid(True) plt.legend() plt.show() # ===== 4) 频移补偿 offset：在时延域乘 e^{j 2π (offset/L) m} 相当于频域平移 offset 个bin ===== m = np.arange(L, dtype=float) ph = np.exp(1j * 2.0 * np.pi * (offset / L) * m).astype(np.complex64) hL_shift = hL * ph # plt.figure(figsize=(8, 4)) # plt.plot(np.abs(hL_shift[:]), label='|h_td| (symbol 0)') # plt.xlabel('Sample index (delay)') # plt.ylabel('Magnitude') # plt.title('Time-domain impulse response magnitude') # plt.grid(True) # plt.legend() # plt.show() # ===== 5) 单位化 FFT 回到 24 点频域 → 得到整条子带的 H_sub ===== H_sub = _fft_u(hL_shift).astype(np.complex64) # shape=(L,) H_sub *= np.sqrt(L / Nd) # ==== 关键2：用导频做全带增益对齐（LS 最小二乘） ==== # （窗口造成的能量截断也会引起幅度偏差，这里用导频矫正） Hp_est = H_sub[dmrs_pos] denom = np.vdot(Hp_est, Hp_est) # conj(Hp_est)·Hp_est if denom != 0: alpha = np.vdot(Hp_est, Hest_dmrs) / denom H_sub *= alpha # ===== 6) 导频一致性：回填原始 LS，确保幅度不缩放 ===== if pilot_consistency: H_sub[dmrs_pos] = Hest_dmrs return H_sub # 1) 在子带频域构建稀疏向量（导频处=LS，其余=0） H_sparse = np.zeros((L,), dtype=np.complex64) H_sparse[dmrs_pos] = Hest_dmrs return H_sub