...
def estimate_symbol_by_blocks1(
    ...
    for (s, e) in runs:
        ...
        y_blk = y[in_blk]
        x_blk = x[in_blk]
        H_ls_denoised = td_dmrs_denoise_fixed(y_blk, x_blk, Nfft=256, window_type="fixed", win_len_ratio=0.16,
                                              eps=1e-12)
        # TODO 改成线性插值后时域降噪
...
def myChannelEstimate_std(rxGrid=None, refInd=None, refSym=None, refGrid=None, carrier=None, prg_gran=24, energy_keep=0.9, guard=0,EST_TFDOMAIN = True, EST_PBCH = False, method = 'interpolation' ,rb_size = 3, N_sym=2,dmrs_l = 0,start_rb=0,end_rb=20):
    cp = "normal" if carrier is None else "extended"
