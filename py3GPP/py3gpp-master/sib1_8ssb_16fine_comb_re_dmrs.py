...
# =============================================================================
# Horizontal physical array/codebook period in the current 4x8 UPA model.
# qh is expressed in this normalized DFT-index domain: qh=0..8 wraps around.
HORIZONTAL_ARRAY_SIZE = 8

# Requested hierarchy:
#   - 8 coarse SSB directions over one horizontal DFT period
#   - 16 fine beam directions over the same period
#
# Therefore, each coarse SSB sector has two fine beam directions.  To keep the
# fine-pair center aligned with the corresponding coarse SSB center, the two
# fine beams are placed at qh_center ± 0.25, since the 16-beam spacing is 0.5
# in the qh domain.
N_COARSE_SSB = 8
N_FINE_BEAMS = 16
COARSE_QH_STEP = HORIZONTAL_ARRAY_SIZE / N_COARSE_SSB       # 1.0
FINE_QH_STEP = HORIZONTAL_ARRAY_SIZE / N_FINE_BEAMS         # 0.5
FINE_PAIR_DELTA_QH = FINE_QH_STEP / 2.0                     # 0.25
...
def build_subarray_dft_vector_64tx(Mv=4, Mh_total=HORIZONTAL_ARRAY_SIZE, Mh_active=HORIZONTAL_ARRAY_SIZE, qv=0, qh_equiv=0.0):
    """
...
def build_upa_dft_vector(Mv=4, Mh=HORIZONTAL_ARRAY_SIZE, qv=0, qh=0):
    """
    Full-aperture 4x8 UPA steering/DFT-like beam.
...
def build_wbf_64x2_subarray(Mv=4, Mh_total=HORIZONTAL_ARRAY_SIZE, Mh_active=HORIZONTAL_ARRAY_SIZE, qv=0, qh_equiv=0.0):
...
def bb_precoder_fixed_phase(phi):
...
def get_bb_precoder(mode: str, prg_idx: int, phase_table):
    ...
    if mode in ("fixed_j", "fixed_pi2"):
        return bb_precoder_fixed_phase(np.pi / 2)

    if mode in ("fixed_minus1", "fixed_pi"):
        return bb_precoder_fixed_phase(np.pi)

    if mode in ("fixed_minusj", "fixed_3pi2"):
        return bb_precoder_fixed_phase(3 * np.pi / 2)

    if mode in ("fixed_1", "fixed_0"):
        return bb_precoder_fixed_phase(0.0)
...
def make_target_mappings_8ssb_16fine(run_all_ssb=False, center_ssb_idx=0, qv=0):
    """
    Build target mappings for an 8-SSB coarse grid and a 16-direction fine grid.

    In the current qh domain:
      coarse SSB centers: qh = ssb_idx * 1.0, ssb_idx = 0..7
      fine directions for one SSB sector: qh_center ± 0.25

    If run_all_ssb=True, return 8 mappings and simulate/average over all coarse
    SSB sectors.  If False, return one mapping around center_ssb_idx for quick
    debugging.
    """
    ssb_indices = range(N_COARSE_SSB) if run_all_ssb else [int(center_ssb_idx) % N_COARSE_SSB]
    mappings = []
    for ssb_idx in ssb_indices:
        qh_center = (ssb_idx * COARSE_QH_STEP) % HORIZONTAL_ARRAY_SIZE
        qh_a = (qh_center - FINE_PAIR_DELTA_QH) % HORIZONTAL_ARRAY_SIZE
        qh_b = (qh_center + FINE_PAIR_DELTA_QH) % HORIZONTAL_ARRAY_SIZE
        # Optional fine direction indices on a shifted 16-beam grid:
        # fine_idx_left/right are mostly for CSV/debug interpretation.
        fine_idx_left = (2 * ssb_idx - 1) % N_FINE_BEAMS
        fine_idx_right = (2 * ssb_idx) % N_FINE_BEAMS
        mappings.append({
            "name": f"SSB{ssb_idx:02d}_center_qh{qh_center:g}_fine_qh{qh_a:g}_qh{qh_b:g}",
            "qv": qv,
            "ssb_idx": ssb_idx,
            "qh_center": qh_center,
            "qh_delta": FINE_PAIR_DELTA_QH,
            "fine_idx_left": fine_idx_left,
            "fine_idx_right": fine_idx_right,
        })
    return mappings
...
def resolve_target_mapping(target_mapping, Mh=HORIZONTAL_ARRAY_SIZE):
    """
    Resolve one target mapping into a center-aligned fine-beam pair.

    Preferred input for 8 coarse SSB / 16 fine beams:
      {"qh_center": 0.0, "qh_delta": 0.25}

    Backward-compatible input:
      {"qh_a": 0, "qh_b": 1}

    Returns:
      qv, qh_a, qh_b, qh_center
    """
    qv = target_mapping.get("qv", 0)

    if "qh_center" in target_mapping:
        qh_center = float(target_mapping["qh_center"]) % Mh
        qh_delta = float(target_mapping.get("qh_delta", FINE_PAIR_DELTA_QH))
...
def make_wbf_list_for_scheme(cfg: SchemeConfig, qv: int, qh_a: float, qh_b: float):
    ...
    if cfg.scheme == "array_coarse_baseline":
        ...
        return [build_wbf_64x2_subarray(4, 8, Mh_active=4, qv=qv, qh_equiv=qh_center)]

    if cfg.scheme == "array_centered_fine":
        # Full-aperture reference beam exactly at the same center as the coarse beam.
        return [build_wbf_64x2_subarray(4, 8, Mh_active=8, qv=qv, qh_equiv=qh_center)]
    ...
    if cfg.scheme == "array_comb_re_dmrs_same_center":
        ...
        return [
            build_wbf_64x2_subarray(4, 8, Mh_active=8, qv=qv, qh_equiv=qh_center),
            build_wbf_64x2_subarray(4, 8, Mh_active=8, qv=qv, qh_equiv=qh_center),
        ]
...
def run_single_trial_global(args):
    ...
    qv, qh_a, qh_b, qh_center = resolve_target_mapping(target_mapping, Mh=8)
...
if __name__ == "__main__":
    ...
    cdl_type = "CDLD"      # e.g. "CDLB", "CDLC", "CDLD"
    tdl_profile = "TDL-C"
    ...
    tdl_directional = False
    tdl_normalize = False
    # Target mapping list for the requested 8-SSB coarse grid / 16-direction fine grid.
    #
    # Default: one coarse SSB sector around qh_center=0 for quick debugging:
    #   coarse/reference center = qh0
    #   fine pair = qh0 ± 0.25  (two of the 16 fine directions)
    #
    ...
    print(f"[Beam grid] N_COARSE_SSB={N_COARSE_SSB}, N_FINE_BEAMS={N_FINE_BEAMS}, "
          f"COARSE_QH_STEP={COARSE_QH_STEP}, FINE_QH_STEP={FINE_QH_STEP}, "
          f"FINE_PAIR_DELTA_QH={FINE_PAIR_DELTA_QH}")
    ...
    scheme_configs = [
        ...
        SchemeConfig(
            label="fixed_j: 16-dir array fine pair around SSB qh_c",
            family="array_based",
            scheme="array_comb_re_dmrs",
            prg_size_rb=48,
            prg_precoding_mode="fixed_j",
            ce_mode="fullband",
            dmrs_pattern="comb2_re",
        ),
        SchemeConfig(
            label="fixed_minus1: 16-dir array fine pair around SSB qh_c",
            family="array_based",
            scheme="array_comb_re_dmrs",
            prg_size_rb=48,
            prg_precoding_mode="fixed_minus1",
            ce_mode="fullband",
            dmrs_pattern="comb2_re",
        ),
        SchemeConfig(
            label="fixed_minusj: 16-dir array fine pair around SSB qh_c",
            family="array_based",
            scheme="array_comb_re_dmrs",
            prg_size_rb=48,
            prg_precoding_mode="fixed_minusj",
            ce_mode="fullband",
            dmrs_pattern="comb2_re",
        ),
