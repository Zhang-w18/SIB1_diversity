...
def _get_decoder(K: int, N: int, list_size: int, crc_degree: str, #ind_iil_inv:None,
    ...
    if dec is None:
        ...
        # print(frozen_pos)
        # print(len(frozen_pos))
        # frozen_pos = np.array(frozen_pos, dtype=np.int32)
        ## TODO 新加的
        # enc = Polar5GEncoder(k=K, n=N)
        dec = Polar5GDecoder(Polar5GEncoder(k=32, n=864,channel_type="downlink"), dec_type="SCL", list_size=8,return_crc_status=True,)
...
def polar_decode_scl_llr(rec_llr: np.ndarray,
    ...
    dec = _get_decoder(K, N, list_size=list_size, crc_degree='CRC24C', use_fast_scl=True , cpu_only=True,return_crc_status=True)
    # ,ind_iil_inv=ind_iil_inv
