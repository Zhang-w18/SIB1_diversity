...
def nrSIB1DMRSIndices_comb2_re_param(
    ...
    """
    Generate two interleaved DMRS index sets with total DMRS density 1/2.

    On each DMRS OFDM symbol, the frequency pattern is:
      k mod 4 = 0 : data associated with beam/precoder state 1
      k mod 4 = 1 : DMRS associated with beam/precoder state 1
      k mod 4 = 2 : data associated with beam/precoder state 2
      k mod 4 = 3 : DMRS associated with beam/precoder state 2

    This corresponds to the conceptual pattern 01020102..., where 1/2 are
    different DMRS states. Each state sees 1/4-density DMRS in frequency.
    """
    Nsc = int(nrb_sib) * 12
    locs = _normalize_dmrs_loc(dmrs_loc, nsym_slot)

    k_b1 = np.arange(1, Nsc, 4, dtype=int)
    k_b2 = np.arange(3, Nsc, 4, dtype=int)
...
def nrSIB1DataIndices_comb2_re_param(nrb_sib: int, nsym_slot: int, dmrs_loc):
    ...
    On DMRS symbols, k mod 4 = 1/3 are DMRS, and k mod 4 = 0/2 are data.
    On non-DMRS symbols, all REs are data. For consistency, they are assigned
    by frequency comb:
    ...
    dmrs_idx_b1, dmrs_idx_b2 = nrSIB1DMRSIndices_comb2_re_param(nrb_sib, nsym_slot, dmrs_loc, "matlab")
...
def bb_precoder_cyclic(prg_idx, phase_table=(0, np.pi / 2, np.pi, 3 * np.pi / 2)):
    phi = phase_table[0 % len(phase_table)]
...
def make_wbf_list_for_scheme(cfg: SchemeConfig, qv: int, qh_a: int, qh_b: int):
    ...
    if cfg.scheme == "array_comb_re_dmrs":
        # RE-level comb DMRS: state 1 and state 2 use two full-aperture fine beams.
        return [
            build_wbf_64x2_subarray(4, 8, Mh_active=8, qv=qv, qh_equiv=qh_a),
            build_wbf_64x2_subarray(4, 8, Mh_active=8, qv=qv, qh_equiv=qh_b),
        ]
    ...
    if cfg.scheme == "precoding_comb_re_dmrs":
        # RE-level comb DMRS: both states share the same spatial DFT beam basis,
        # while state 1/state 2 use different dual-pol baseband precoders.
        Wbf = build_wbf_64x2_upa(qv1=qv, qh1=qh_a, qv2=qv, qh2=qh_a)
        return [Wbf, Wbf]
...
if __name__ == "__main__":
    ...
    target_mappings = [
        {"name": "Pair_qh0_qh1", "qv": 0, "qh_a": 0, "qh_b": 0}, # gai
    ]
    ...
    # -------------------------------------------------------------------------
    scheme_configs = [
        # RE-level comb DMRS: two array fine beams are interleaved at RE level.
        SchemeConfig(
            label="Comb-DMRS RE-level: array beam1/beam2, pattern 01020102",
            family="array_based",
        ...
        # RE-level comb DMRS: same spatial beam basis, two dual-pol precoder states.
        # State 1 uses [1,1]/sqrt(2); state 2 uses [1,j]/sqrt(2).
        SchemeConfig(
            label="Comb-DMRS RE-level: precoding states [1,1]/[1,j]",
        ...
        # Legacy reference: one selected full-aperture fine beam, legacy 010101... DMRS.
        SchemeConfig(
            label="Legacy: array refined fine beam, legacy DMRS 010101",
            family="array_based",
    ...
    out_csv = "sib1_comb_re_dmrs_bler.csv"
    out_png = "sib1_comb_re_dmrs_bler.png"
