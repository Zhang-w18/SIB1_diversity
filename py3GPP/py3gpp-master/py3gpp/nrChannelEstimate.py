    ...
    if True:
        ...
        for col in range(N):
            ...
            if len(idx) > 0:
                ...
                real_interp = np.interp(np.arange(K), sym_idx, np.real(rxSym[idx] / refSym[idx]))
                imag_interp = np.interp(np.arange(K), sym_idx, np.imag(rxSym[idx] / refSym[idx]))
                H[:, col] = real_interp + 1j * imag_interp
        ...
        if interp_vertical:
            for row in range(K):
                ...
                real_interp = np.interp(np.arange(N), idx, np.real(temp[idx]))
                imag_interp = np.interp(np.arange(N), idx, np.imag(temp[idx]))
                H[row, :] = real_interp + 1j * imag_interp
