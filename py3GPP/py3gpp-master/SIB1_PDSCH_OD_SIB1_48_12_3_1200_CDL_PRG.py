...
def _rx_round_get_llr(
    ...
    for r in range(rxGrids.shape[0]):
        ...
        # ==================== 需要修改的地方 ====================
        if ce_mode == "fullband":
            # 核心修改：初始化一个空 H_r，然后遍历 PRG，对每个 bundle 独立调用 fullband 方法
            ...
            for rb0, rb1 in iter_prg_ranges(nrb_sib, prg_size_rb):
                H_bundle = myChannelEstimate_std(
                    ...
                    dmrs_l=dmrs_cols,
                    start_rb=rb0,
                    end_rb=rb1,  # 限制在当前的 bundle 边界内
                    td_win_len_ratio=0.08,
                    td_window_type="fixed",
                )
                # 把该 bundle 估出来的信道拼接到全带宽网格对应的位置
                sc0, sc1 = rb0 * 12, rb1 * 12
...
if __name__ == "__main__":
    ...
    # 核心修改：定义要对比的 Bundle Size 配置列表
    # 格式: (图例标签, Precoding模式, CE模式, PRG大小)
    # 这里默认使用 Cyclic Precoding 来验证不同分块大小下的频率分集效果
    configurations = [
        ("Cyclic PRG=6 (Bundle-TD CE)", "cyclic", "fullband", 6),
        ("Cyclic PRG=8 (Bundle-TD CE)", "cyclic", "fullband", 8),
        ("Cyclic PRG=12 (Bundle-TD CE)", "cyclic", "fullband", 12),
        ("Cyclic PRG=24 (Bundle-TD CE)", "cyclic", "fullband", 24),

        # 可选：如果你想加一条 48RB 全频带不分块的 Benchmark 上限曲线，可以解开下面这行的注释
        # ("Traditional PRG=48 (Fullband CE)", "traditional", "fullband", 48),
    ]

    for label, prg_mode, ce_mode, prg_size in configurations:
        bler_curve = simulate_bler_sector_smart(
            snr_db_list=snr_points,
            n_trials_per_beam=n_trials,
            ncellid=208,
            fs=61.44e6,  # 如果爆内存，记得改成 30.72e6
            scs_khz=30,
            seed=2029,
            nrb_sib=48,
            nsym_sib=12,
            N_comb=1,
            payload=1200,
            prg_size_rb=prg_size,  # <--- 动态传入不同的 Bundle Size
            prg_precoding_mode=prg_mode,
            ssb_beams=sector_beams,
            ce_mode=ce_mode,  # 传入 fullband，底层会按 PRG 分块做 TD 降噪
        )
    ...
    for label, bler_curve in curves.items():
        # 画图，截断极小值到 1e-4 防止在对数坐标轴下报错
        plt.semilogy(snr_points, np.maximum(bler_curve, 1e-4), marker='o', label=label)

    plt.grid(True, which="both")
    plt.xlabel("SNR (dB)")
    plt.ylabel("Sector Average BLER")
    plt.title("SIB1 BLER vs Bundle Size (Cyclic Precoding + Bundle-TD CE)")
    plt.legend(loc="best")
    plt.tight_layout()
    plt.savefig("prg_precoding_bundle_size_bler.png", dpi=150)

    with open("prg_precoding_bundle_size_bler.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    print("不同 Bundle Size 的扇区平均 BLER 曲线已保存到 prg_precoding_bundle_size_bler.png")