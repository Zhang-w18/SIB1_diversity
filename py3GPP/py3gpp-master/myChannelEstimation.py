...
def myChannelEstimate_std(rxGrid=None, refInd=None, refSym=None, refGrid=None, carrier=None, prg_gran=24, energy_keep=0.9, guard=0,EST_TFDOMAIN = True, EST_PBCH = False, method = 'interpolation' ,rb_size = 3, N_sym=2,dmrs_l = 0):
    cp = "normal" if carrier is None else "extended"
        ...
        for col in range(N):
            if EST_PBCH: #PBCH，逐块进行变换域降噪
                H_col, ok = estimate_symbol_by_blocks(
                    col=col, K=K, refInd=refInd, rxSym_all=rxSym_all, refSym_all=refSym_all
                )
