...
# ============================ 基本波形函数 =============================
def prach_symbol(X_fd):
    """iFFT 到时域。这里按 '局部N点IFFT' 模式做短序列调制"""
    return np.fft.ifft(X_fd)
...
def fd_corr_ifft(y_td, Xref_fd):
    """频域乘共轭 + IFFT = 循环相关（检测时延）"""
    Y = np.fft.fft(y_td)
    Z = Y * np.conj(Xref_fd)
...
# ============================ TDL-C 信道（占位实现） =============================
# class ChannelInfo:
#     def __init__(self, nTx, nRx, Speed, Fc, Fs, ChanType, DS, NFFT, TO, FO):
#         self.nTx = nTx; self.nRx = nRx
#         self.Speed = Speed; self.Fc = Fc; self.Fs = Fs
#         self.ChanType = ChanType; self.DS = DS
#         self.NFFT = NFFT; self.TO = TO; self.FO = FO
#
#         L = 18
#         delays = np.arange(L)
#         pdp = np.exp(-delays/4.0)
#         pdp = pdp / np.sum(pdp)
#         self.time_channel = np.zeros((nRx, nTx, L, 1), dtype=complex)
#         h0 = (rng.normal(size=L) + 1j*rng.normal(size=L)) / np.sqrt(2)
#         h0 = h0 * np.sqrt(pdp * L)
#         self.time_channel[0, 0, :, 0] = h0
#
#     def gen_Rayleigh(self, num_samples=1):
#         pass
#
...
#         np.random.seed(seed)
#     ch = ChannelInfo(
#         nTx=1, nRx=1, Speed=speed_kmh, Fc=fc_hz, Fs=fs_hz,
#         ChanType='TDLC', DS=ds, NFFT=nfft, TO=0, FO=0
#     )
...
#         fs_hz=fs_hz, ds=ds, fc_hz=fc_hz, speed_kmh=speed_kmh,
#         nfft=nfft, seed=seed, normalize=normalize
#     )
...
# === ADDED: 去噪版 CFO 估计 ===
def estimate_cfo_denoised(rx_no_cp: np.ndarray, L_rep: int, ma_len: int = 64, use_hann: bool = True):
    """
    思路：
      1) 对相邻重复块，逐样本构造相位差 phasor[n] = blk_{k+1}[n] * conj(blk_k[n])
      2) 对 phasor 做滑动平均（moving average），并可叠加 Hann 窗，降低噪声与边缘泄漏
      3) 跨所有相邻块对把平滑后的 phasor 累加，再取 angle 得到 Δφ
      4) 归一化：cfo_hat = Δφ / (2π)，单位为“子载波间隔的归一化 CFO”
    """
    K = rx_no_cp.size // L_rep
    if K < 2:
        return 0.0, 0.0
    ...
    # 滤波核
    if ma_len > 1:
        ma = np.ones(ma_len, dtype=float) / ma_len
    else:
        ma = None
    win = np.hanning(L_rep) if use_hann else np.ones(L_rep, dtype=float)

    acc = 0+0j
    for k in range(K - 1):
        blk0 = rx_no_cp[k*L_rep:(k+1)*L_rep]
        blk1 = rx_no_cp[(k+1)*L_rep:(k+2)*L_rep]
        phasor = blk1 * np.conj(blk0)               # 逐样本相位差
        if ma is not None:                          # 滑动平均去噪
            phasor = np.convolve(phasor, ma, mode="same")
        phasor *= win                               # 边缘加窗，减小泄漏
        acc += np.sum(phasor)

    dphi = np.angle(acc) % (2*np.pi)
    cfo_hat = dphi / (2*np.pi)
    return cfo_hat, dphi
...
def simulate_preamble_error_curve(
    ...
    # det_threshold, p_fa_emp = calibrate_threshold_for_fa(
    #     cfg,
    #     bank_fd,
    #     mapping,
    #     target_fa=target_fa,
...
if __name__ == "__main__":
    ...
    # cfg = CFG_B4
    cfg = CFG_F0
    snr_list = np.arange(-16,4, 2)  # -12, -10, ..., 12 dB
