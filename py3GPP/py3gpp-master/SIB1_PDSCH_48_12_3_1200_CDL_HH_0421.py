...
# def build_upa_dft_vector(Mv=4, Mh=4, qv=0, qh=0):
...
#
#     # vertical-major, horizontal-fast
#     v2d = np.kron(vv, vh)
#
...
def cdl_c_impulse_response_32t4r_full(
    ...
    h00 = channel_time_interpolation(
        delays,
        h_paths[:, 0, 0],
        ts=1.0 / fs_hz,
        num_ext=4
    ...
    n_rx = h_paths.shape[1]
    n_tx = h_paths.shape[2]

    assert n_rx == 4, f"Expected 4Rx, got {n_rx}"
    assert n_tx == 32, f"Expected 32Tx for 4x4x2, got {n_tx}"
...
def one_shot_sib_min_combined(
    ...
    Wbf = build_wbf_from_channel_codebook(qv=qv, qh=qh, M=4, N=4, P=2)
    llr_sum = None
