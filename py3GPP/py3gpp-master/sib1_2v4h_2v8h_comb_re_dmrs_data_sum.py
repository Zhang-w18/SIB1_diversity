...
# Trial-level CDL AOD randomization.  The horizontal AOD of the CDL channel can
# be randomly rotated within half of the coarse horizontal main-lobe width so
# one BLER point represents random UE locations inside the selected SSB sector.
# IMPORTANT: the actual random angle is generated from the trial seed, not from
# the scheme, so the same SNR/target/trial index produces the same angle_gap for
# every scheme.
TX_DH_LAMBDA = 0.5
TX_DV_LAMBDA = 0.8  # documented target configuration; channel_initialize may still hard-code dV unless modified there.
COARSE_HPBW_DEG = 57.3 / (TX_DH_LAMBDA * COARSE_MH_ACTIVE)
DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG = 15#0 * COARSE_HPBW_DEG  # ~= 14.325 deg for 4H, dH=0.5
...
class SchemeConfig:
    ...
    # comb2_re only:
    #   "split" -> existing behavior: data_idx_b1 uses H1, data_idx_b2 uses H2
    #   "sum"   -> new behavior: all data REs use unit-power coherent sum c1*H1+c2*H2
    comb_data_mode: str = "split"
...
def make_wbf_list_for_scheme(cfg: SchemeConfig, qv: int, qh_a: float, qh_b: float, qh_center: Optional[float] = None):
    ...
    if cfg.scheme in ("array_comb_re_dmrs", "array_comb_re_dmrs_data_sum"):
    ...
    if cfg.scheme in ("precoding_comb_re_dmrs", "precoding_comb_re_dmrs_data_sum"):
...
def get_comb2_state_effective_vectors(
    Wbf_list,
    prg_precoding_mode="traditional",
    phase_table=(0, np.pi / 2, np.pi, 3 * np.pi / 2),
):
    """
    Return unit-norm effective TX vectors for comb state 1 and state 2.

    State 1 uses traditional [1,1].
    State 2 uses [1,j] when prg_precoding_mode == "cyclic" with phase_table[1];
    otherwise it also uses traditional [1,1].
    """
    if Wbf_list is None or len(Wbf_list) == 0:
    ...
    n1 = np.linalg.norm(w_eff_b1)
    n2 = np.linalg.norm(w_eff_b2)
    if n1 < 1e-12 or n2 < 1e-12:
        raise ValueError("Near-zero comb state effective TX vector; check Wbf/precoder.")

    return w_eff_b1 / n1, w_eff_b2 / n2


def get_unit_power_sum_coeffs(w_eff_b1, w_eff_b2):
    """
    口径A：先用 a1=a2=1/sqrt(2) 做 two-state data superposition，
    再对最终 composite TX vector 做单位功率归一化。

      w_data = c1*w_eff_b1 + c2*w_eff_b2
      H_eff  = c1*H1 + c2*H2
    """
    a1 = 1.0 / np.sqrt(2.0)
    a2 = 1.0 / np.sqrt(2.0)

    w_sum = a1 * w_eff_b1 + a2 * w_eff_b2
    norm_sum = np.linalg.norm(w_sum)
    if norm_sum < 1e-12:
        raise ValueError("Composite data precoder has near-zero norm; check beam/state pair.")

    return a1 / norm_sum, a2 / norm_sum
...
def build_precoded_tx_grids_comb2_re_dmrs_data_sum(
    base_grid,
    meta2,
    Wbf_list,
    prg_precoding_mode="traditional",
    ...
    """
    New comb-DMRS data-sum scheme.

    DMRS REs:
      - state-1 DMRS uses state-1 vector only.
      - state-2 DMRS uses state-2 vector only.

    Data REs:
      - every SIB1 data RE uses the same unit-power composite vector
        w_data = c1*w_eff_b1 + c2*w_eff_b2, with口径A normalization.
    """
    w_eff_b1, w_eff_b2 = get_comb2_state_effective_vectors(
        Wbf_list=Wbf_list,
        prg_precoding_mode=prg_precoding_mode,
        phase_table=phase_table,
    )
    c1, c2 = get_unit_power_sum_coeffs(w_eff_b1, w_eff_b2)
    # get_unit_power_sum_coeffs() has already normalized the composite vector to unit power.
    # Do not renormalize again, otherwise the Rx H_eff coefficients would not exactly match Tx.
    w_data = c1 * w_eff_b1 + c2 * w_eff_b2
    ...
    # 1) All data REs use simultaneous two-state transmission.
    data_idx = np.asarray(meta2["sib1_idx"], dtype=int)
    k_data, l_data = data_idx % K, data_idx // K
    tx_grids[:, k_data, l_data] = w_data[:, None] * base_grid[k_data, l_data][None, :]

    # 2) DMRS REs remain state-specific so Rx can estimate H1 and H2 separately.
    dmrs_idx_b1 = np.asarray(meta2["dmrs_idx_b1"], dtype=int)
    dmrs_idx_b2 = np.asarray(meta2["dmrs_idx_b2"], dtype=int)

    k1, l1 = dmrs_idx_b1 % K, dmrs_idx_b1 // K
    k2, l2 = dmrs_idx_b2 % K, dmrs_idx_b2 // K
...
def cdl_impulse_response_96t4r_full(
    fs_hz,
    ds_ns=300,
    fc_GHz=7,
    seed=None,
...
def apply_cdl_mimo_tx(
    ...
    H_mimo = cdl_impulse_response_96t4r_full(
        fs_hz,
        ds_ns,
        fc_hz / 1e9,
        seed,
...
def estimate_comb2_re_dmrs_two_state_ce(
    rxGrid,
    refGridH_b1,
    refGridH_b2,
    meta2,
    ...
    """
    Estimate H1 and H2 separately from comb-DMRS state-1/state-2 pilots.
    Return full-grid H1_all and H2_all.
    """
...
def estimate_comb2_re_dmrs_reuse_existing_ce(
    ...
    """
    Existing split-data comb-DMRS behavior:
      data_idx_b1 -> H1
      data_idx_b2 -> H2
    """
    H1_all, H2_all = estimate_comb2_re_dmrs_two_state_ce(
        rxGrid=rxGrid,
...
def estimate_comb2_re_dmrs_data_sum_ce(
    rxGrid,
    refGridH_b1,
    refGridH_b2,
    meta2,
    ...
    """
    New data-sum comb-DMRS receiver:
      H_eff = c1*H1 + c2*H2 on every SIB1 data RE.
    """
    H1_all, H2_all = estimate_comb2_re_dmrs_two_state_ce(
        rxGrid=rxGrid,
        refGridH_b1=refGridH_b1,
    ...
    data_idx = np.asarray(meta2["sib1_idx"], dtype=int)
    k_data, l_data = data_idx % K, data_idx // K
    H_eff[k_data, l_data] = c1 * H1_all[k_data, l_data] + c2 * H2_all[k_data, l_data]
...
def _rx_round_get_llr(
    ...
    comb_data_mode="split",
):
    if meta2 is not None and meta2.get("dmrs_pattern") == "comb2_re":
        if comb_data_mode == "sum":
            tx_grids = build_precoded_tx_grids_comb2_re_dmrs_data_sum(
                sib_grid,
                meta2,
                Wbf_list,
                prg_precoding_mode=prg_precoding_mode,
            ...
            raise ValueError(f"Unsupported comb_data_mode: {comb_data_mode}")
    else:
    ...
    if meta2.get("dmrs_pattern") == "comb2_re":
        ...
        for r in range(rxGrids.shape[0]):
            if comb_data_mode == "sum":
                H_eff = estimate_comb2_re_dmrs_data_sum_ce(
                    rxGrid=rxGrids[r],
                    refGridH_b1=refGridH_b1,
                    refGridH_b2=refGridH_b2,
                    meta2=meta2,
...
def one_shot_sib_min_combined(
    ...
    for r in range(N_comb):
        ...
        llr_r = _rx_round_get_llr(
            ...
            angle_gap_deg=angle_gap_deg,
            td_win_len_ratio=td_win_len_ratio,
            comb_data_mode=scheme_cfg.comb_data_mode,
        )
        ...
        for snr_db in snr_db_list:
            ...
            for mapping in target_mappings:
                ...
                for _ in range(n_trials_per_target):
                    ...
                    angle_gap_deg = deterministic_angle_gap_from_trial_seed(
                        trial_seed,
...
if __name__ == "__main__":
    ...
    if channel_model.upper() == "TDL":
        ...
        snr_points = [-17,-15, -13, -11, -9, -7, -5, -3,-1]#np.arange(-21, 1, 2.0).tolist()
    td_win_sweep = [0.12]
    tdl_directional = False
    tdl_normalize = False
    ...
    print(f"[CDL random AOD rotation] enabled={ENABLE_RANDOM_ANGLE_GAP_BY_DEFAULT}, "
          f"angle_gap ~ Uniform(-{DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG:.3f}, +{DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG:.3f}) deg, "
          f"COARSE_HPBW={COARSE_HPBW_DEG:.3f} deg")
    ...
    scheme_configs = [
        # ================================================================
        # RE-level comb-DMRS schemes
        #   - total DMRS RE overhead unchanged vs legacy DMRS union
        #   - each state sees 1/4-density DMRS
        # ================================================================
        SchemeConfig(
            label="Comb-DMRS-array-data-sum: qh_a/qh_b simultaneous data, coherent H1+H2",
            family="array_based",
            scheme="array_comb_re_dmrs_data_sum",
            prg_size_rb=48,
            prg_precoding_mode="traditional",
            ce_mode="fullband",
            dmrs_pattern="comb2_re",
            td_win_len_ratio=0.24,
            comb_data_mode="sum",
        ),
        SchemeConfig(
            label="Comb-DMRS-precoding-data-sum: [1,1]/[1,j] simultaneous data, coherent H1+H2",
            family="precoding_based",
            scheme="precoding_comb_re_dmrs_data_sum",
            prg_size_rb=48,
    ...
    out_csv = "sib1_2v4h_2v8h_comb_re_dmrs_with_prg_random_angle_bler_sum.csv"
    out_png = "sib1_2v4h_2v8h_comb_re_dmrs_with_prg_random_angle_bler_sum.png"
        ...
        for cfg in scheme_configs:
            for snr, bler_val in zip(snr_points, curves[cfg.label]):
                writer.writerow([
                    ...
                    channel_model,
                    cdl_type,
                    ENABLE_RANDOM_ANGLE_GAP_BY_DEFAULT if channel_model.upper() == "CDL" else False,
                    DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG if channel_model.upper() == "CDL" else 0.0,
                    snr,
                    bler_val,
                ])
