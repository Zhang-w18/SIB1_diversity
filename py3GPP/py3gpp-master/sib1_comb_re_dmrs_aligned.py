...
def circular_mid_qh(qh_a, qh_b, Mh=8):
    """
...
def resolve_target_mapping(target_mapping, Mh=8):
    """
    Resolve one target mapping into a center-aligned fine-beam pair.

    Preferred input:
      {"qh_center": 0.5, "qh_delta": 0.5}
    ...
    if "qh_center" in target_mapping:
        ...
        qh_delta = float(target_mapping.get("qh_delta", 0.5))
...
def make_wbf_list_for_scheme(cfg: SchemeConfig, qv: int, qh_a: float, qh_b: float):
    ...
    """
    qh_center = circular_mid_qh(qh_a, qh_b, Mh=8)
...
def _rx_round_get_llr(
    ...
    PRINT_DIAG = True
    if PRINT_DIAG:
        qh_best, qh_grid, gains = scan_array_beam_gain_from_H(
            H_mimo,
            qv=main_qv,
            qh_grid=np.arange(-1.5, 1.5001, 0.05),
            Mh_active=8,
        )
        print(f"[Beam scan] best qh={qh_best:.3f}")
...
def scan_array_beam_gain_from_H(
    H_mimo,
    qv=0,
    qh_grid=None,
    Mh_total=8,
    ...
    """
    Scan effective beam gain for array/subarray-based beams.

    H_mimo shape: [n_rx, n_tx, n_tap]
    gain(qh) = sum_{rx,tap} | sum_tx H[rx, tx, tap] * w[tx] |^2
    """
    if qh_grid is None:
        qh_grid = np.arange(-2.0, 2.0001, 0.05)

    gains = []

    for qh in qh_grid:
        Wbf = build_wbf_64x2_subarray(
            Mv=4,
            Mh_total=Mh_total,
            Mh_active=Mh_active,
            qv=qv,
        ...
        # H_eff[rx, tap] = H[rx, tx, tap] @ w_eff[tx]
    ...
    return float(qh_grid[best_idx]), qh_grid, gains
# =============================================================================
# Main demo
# =============================================================================
if __name__ == "__main__":
    ...
    target_mappings = [
        # Center-aligned definition:
        #   coarse/reference center = qh_center = 0.5
        #   fine pair = qh_center ± qh_delta = qh0 and qh1
        {"name": "Center_qh0p5_pair_qh0_qh1", "qv": 0, "qh_center": 0, "qh_delta": 0.25},
    ]
    ...
    # -------------------------------------------------------------------------
    scheme_configs = [
        # Coarse/reference beam centered at qh_center.
        SchemeConfig(
            label="Aligned reference: 4-ant coarse beam centered at qh_c",
            family="array_based",
        ...
        # Full-aperture single beam centered at the same qh_center.
        SchemeConfig(
            label="Aligned reference: 8-ant centered fine beam, legacy DMRS",
            family="array_based",
        ...
        # SchemeConfig(
        #     label="Comb-DMRS control: array same centered beam/state1=state2",
        #     family="array_based",
        ...
        # Two array fine beams whose midpoint equals the coarse/reference center.
        SchemeConfig(
            label="Comb-DMRS aligned: array fine pair centered at qh_c",
            family="array_based",
        ...
        # SchemeConfig(
        #     label="Comb-DMRS control: precoding same centered state [1,1]/[1,1]",
        ...
        SchemeConfig(
            label="Comb-DMRS aligned: precoding centered states [1,1]/[1,j]",
            family="precoding_based",
            scheme="precoding_comb_re_dmrs",
            prg_size_rb=48,
    ...
    out_csv = "sib1_comb_re_dmrs_aligned_bler.csv"
    out_png = "sib1_comb_re_dmrs_aligned_bler.png"
    ...
    plt.title(f"SIB1 BLER: RE-level Comb DMRS Fine Beam ({channel_model}/{tdl_profile if channel_model.upper()=='TDL' else cdl_type})")
