...
def run_single_trial_global(args):
    ...
    ok, _ = one_shot_sib_min_combined(
        ...
        phase_table=(0, np.pi / 4, np.pi / 2, 3*np.pi / 4),
...
if __name__ == "__main__":
    ...
    configurations = [
        ("4 phase Cyclic PRG=6 (Bundle-TD CE)", "cyclic", "fullband", 6),
        ("4 phase Cyclic PRG=12 (Bundle-TD CE)", "cyclic", "fullband", 12),
    ...
    plt.legend(loc="best")
    plt.tight_layout()
    plt.savefig("prg_precoding_bundle_size_bler2.png", dpi=150)

    with open("prg_precoding_bundle_size_bler2.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    print("不同 Bundle Size 的扇区平均 BLER 曲线已保存到 prg_precoding_bundle_size_bler2.png")