...
# =============================================================================
# Configurable beam-grid / antenna configuration
# =============================================================================
# Supported NR baseline beam configurations using one common physical aperture:
#   physical TX array is fixed to (M,N,P) = (6,16,2) for all cases.
#   8  total beams = 2V x 4H  codebook, active subarray Np=4
#   16 total beams = 2V x 8H  codebook, active subarray Np=8
#   32 total beams = 2V x 16H codebook, active subarray Np=16
# with (dH, dV) = (0.5, 0.8) lambda.
#
# IMPORTANT:
#   The script now supports changing the beam configuration at runtime via
#   configure_beam_grid(total_beams).  Because ProcessPoolExecutor on Windows
#   starts fresh Python processes, run_single_trial_global() also calls
#   configure_beam_grid(static_cfg["beam_config_total"]) inside each worker.
TX_VERTICAL_SIZE = 6
# Common physical horizontal aperture for all 8/16/32-beam baseline cases.
# Only the active horizontal subarray width Np changes: 4, 8, or 16.
COMMON_PHYSICAL_HORIZONTAL_SIZE = 16
NUM_POLARIZATIONS = 2
N_COARSE_V = 2
N_FINE_V = 2
TX_DH_LAMBDA = 0.5
...
SUPPORTED_BEAM_CONFIG_TOTALS = (8, 16, 32)
CURRENT_BEAM_CONFIG_TOTAL = 16

# CDL angle convention used in this script.  To avoid the previously observed
# negative-azimuth / 350-degree antenna-pattern issue, baseline sweeps use a
# positive absolute AoD region: AOD_BASE_DEG +/- AOD_RANDOM_SPAN_DEG.
AOD_BASE_DEG = 30.0
# In the user's current channel_initialize.py, the positive absolute CDL AoD base
# is already handled there. Therefore this script should normally pass only the
# trial-level offset to channel_initialize(angle_gap=...).  Set True only if
# channel_initialize has NOT been modified and expects the full absolute AoD here.
PASS_ABSOLUTE_AOD_TO_CHANNEL = False

# Keep the same UE angular spread for all baseline cases.
# This compares robustness of 4H/8H/16H active apertures under the same +/-15 deg sector spread.
USE_HALF_HPBW_RANDOM_SPAN = False
AOD_RANDOM_SPAN_DEG = 0.0
ENABLE_RANDOM_ANGLE_GAP_BY_DEFAULT = True
TD_WIN_LEN_RATIO = 0.12
FINE_PAIR_MODE = "symmetric_halfstep"

# The following globals are initialized by configure_beam_grid().
HORIZONTAL_ARRAY_SIZE = None
TX_PORTS = None
N_COARSE_H = None
N_FINE_H = None
N_COARSE_SSB = None
...
FINE_QV_STEP = None
FINE_QH_STEP = None
COARSE_MH_ACTIVE = None
FINE_MH_ACTIVE = None
COARSE_HPBW_DEG = None
DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG = AOD_RANDOM_SPAN_DEG


def configure_beam_grid(total_beams: int = 16):
    """
    Configure the SSB beam grid for a total beam count on a common 16H physical aperture.

    total_beams = 8/16/32 means:
      - vertical beams: 2
      - horizontal beams: total_beams / 2 = 4/8/16
      - physical horizontal elements N is always 16
      - active horizontal subarray width Np is 4/8/16 respectively

    Therefore:
      8 beams  -> COARSE_QH_STEP = 16 / 4  = 4,  qh centers 0,4,8,12
      16 beams -> COARSE_QH_STEP = 16 / 8  = 2,  qh centers 0,2,...,14
      32 beams -> COARSE_QH_STEP = 16 / 16 = 1,  qh centers 0,1,...,15
        """
    global CURRENT_BEAM_CONFIG_TOTAL
    global HORIZONTAL_ARRAY_SIZE, TX_PORTS
    global N_COARSE_H, N_FINE_H, N_COARSE_SSB, N_FINE_BEAMS
    global COARSE_QV_STEP, COARSE_QH_STEP, FINE_QV_STEP, FINE_QH_STEP
    global COARSE_MH_ACTIVE, FINE_MH_ACTIVE, COARSE_HPBW_DEG
    global DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG

    total_beams = int(total_beams)
    if total_beams not in SUPPORTED_BEAM_CONFIG_TOTALS:
        raise ValueError(f"Unsupported total_beams={total_beams}; expected one of {SUPPORTED_BEAM_CONFIG_TOTALS}")
    if total_beams % N_COARSE_V != 0:
        raise ValueError("total_beams must be divisible by N_COARSE_V=2")

    CURRENT_BEAM_CONFIG_TOTAL = total_beams
    N_COARSE_H = total_beams // N_COARSE_V

    # Fair baseline sweep: keep one common 16H physical array and only change
    # the active horizontal subarray width/codebook density.
    HORIZONTAL_ARRAY_SIZE = COMMON_PHYSICAL_HORIZONTAL_SIZE
    N_FINE_H = N_COARSE_H

    TX_PORTS = TX_VERTICAL_SIZE * HORIZONTAL_ARRAY_SIZE * NUM_POLARIZATIONS
    N_COARSE_SSB = N_COARSE_V * N_COARSE_H
    N_FINE_BEAMS = N_FINE_V * N_FINE_H

    COARSE_QV_STEP = TX_VERTICAL_SIZE / N_COARSE_V
    COARSE_QH_STEP = HORIZONTAL_ARRAY_SIZE / N_COARSE_H
    FINE_QV_STEP = TX_VERTICAL_SIZE / N_FINE_V
    FINE_QH_STEP = HORIZONTAL_ARRAY_SIZE / N_FINE_H

    # Active aperture width Np changes with the horizontal beam count.
    # The physical aperture remains 16H; build_subarray_dft_vector_tx() centers
    # the active subarray inside the 16H aperture.
    COARSE_MH_ACTIVE = N_COARSE_H
    FINE_MH_ACTIVE = N_FINE_H
    COARSE_HPBW_DEG = 57.3 / (TX_DH_LAMBDA * COARSE_MH_ACTIVE)

    # User-requested common angular spread: always +/-15 deg for 8/16/32 cases.
    if USE_HALF_HPBW_RANDOM_SPAN:
        DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG = 0.5 * COARSE_HPBW_DEG
    else:
        DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG = AOD_RANDOM_SPAN_DEG


def default_center_ssb_index_for_aod(aod_deg: float = AOD_BASE_DEG):
    """
    Choose the horizontal SSB index whose DFT qh is closest to the configured
    absolute AoD.  Empirically in the current CDL/array coordinate convention,
    positive AoD maps to negative qh.

    qh_center ~= -N*dH*sin(AoD), modulo N, where N is the common physical 16H aperture.
    For AoD=30 deg and N=16, qh≈12 for all three baseline cases.
    The selected horizontal SSB index is therefore:
      2Vx4H  -> h_idx = round(12 / 4) = 3,  qh_center = 12
      2Vx8H  -> h_idx = round(12 / 2) = 6,  qh_center = 12
      2Vx16H -> h_idx = round(12 / 1) = 12, qh_center = 12
    """
    qh_center = (-HORIZONTAL_ARRAY_SIZE * TX_DH_LAMBDA * np.sin(np.deg2rad(float(aod_deg)))) % HORIZONTAL_ARRAY_SIZE
    h_idx = int(np.round(qh_center / COARSE_QH_STEP)) % N_COARSE_H
    v_idx = 0
    return v_idx * N_COARSE_H + h_idx


# Initialize the default configuration for module import.
configure_beam_grid(CURRENT_BEAM_CONFIG_TOTAL)
...
def build_subarray_dft_vector_tx(Mv=TX_VERTICAL_SIZE, Mh_total=HORIZONTAL_ARRAY_SIZE, Mh_active=FINE_MH_ACTIVE, qv=0, qh_equiv=0.0, center_active=True):
    """
    ...
    if center_active:
        # Use a centered horizontal subarray for coarse beams so the phase center is
        # aligned with the full-aperture fine beam. For Mh_total=8 and Mh_active=4,
        # this selects columns 2,3,4,5 instead of the left-edge columns 0,1,2,3.
        start = int((Mh_total - Mh_active) // 2)
    else:
        start = 0
    active_pos = start + m_active
    vh[active_pos.astype(int)] = np.exp(1j * 2.0 * np.pi * active_pos * qh_equiv / Mh_total) / np.sqrt(Mh_active)
...
def map_spatial_beam_to_dualpol_tx(v_spatial, pol_idx=0):
...
def build_wbf_tx_x2_subarray(Mv=TX_VERTICAL_SIZE, Mh_total=HORIZONTAL_ARRAY_SIZE, Mh_active=FINE_MH_ACTIVE, qv=0, qh_equiv=0.0, center_active=True):
    """TX_PORTS x 2 dual-pol Wbf for array/subarray-based beam."""
    v_sp = build_subarray_dft_vector_tx(Mv, Mh_total, Mh_active, qv, qh_equiv, center_active=center_active)
    return np.column_stack([
        map_spatial_beam_to_dualpol_tx(v_sp, pol_idx=0),
        map_spatial_beam_to_dualpol_tx(v_sp, pol_idx=1),
    ])


def build_wbf_tx_x2_upa(qv1=0, qh1=0, qv2=None, qh2=None):
    """
    TX_PORTS x 2 dual-pol Wbf for precoding-based beam-state variation.
    ...
    w1 = map_spatial_beam_to_dualpol_tx(v1, pol_idx=0)
    w2 = map_spatial_beam_to_dualpol_tx(v2, pol_idx=1)
...
class SchemeConfig:
    ...
    prg_precoding_mode: str = "traditional"
    ce_mode: str = "fullband"
    precoding_basis: str = "polarization"
    dmrs_pattern: str = "legacy"
    td_win_len_ratio: float = TD_WIN_LEN_RATIO
...
def make_wbf_list_for_scheme(cfg: SchemeConfig, qv: int, qh_a: float, qh_b: float, qh_center: Optional[float] = None):
    """
    ...
    """
    if qh_center is None:
        # Only use the circular midpoint for symmetric/manual beam pairs.  For the
        # 2Vx4H/2Vx8H center_plus mapping, qh_a/qh_b are associated fine beams and
        # are not symmetric around the SSB center; qh_center must be preserved.
        qh_center = circular_mid_qh(qh_a, qh_b, Mh=HORIZONTAL_ARRAY_SIZE)
    else:
        qh_center = float(qh_center) % HORIZONTAL_ARRAY_SIZE

    if cfg.scheme == "array_coarse_baseline":
        # Coarse beam: reduced aperture, centered between the two fine beams.
        return [build_wbf_tx_x2_subarray(TX_VERTICAL_SIZE, HORIZONTAL_ARRAY_SIZE, Mh_active=COARSE_MH_ACTIVE, qv=qv, qh_equiv=qh_center)]

    if cfg.scheme == "array_centered_fine":
        # Full-aperture reference beam exactly at the same center as the coarse beam.
        return [build_wbf_tx_x2_subarray(TX_VERTICAL_SIZE, HORIZONTAL_ARRAY_SIZE, Mh_active=FINE_MH_ACTIVE, qv=qv, qh_equiv=qh_center)]

    if cfg.scheme == "array_fdm_fine":
        # FDM fine-beam refinement: two full-aperture fine beams centered around qh_center.
        return [
            build_wbf_tx_x2_subarray(TX_VERTICAL_SIZE, HORIZONTAL_ARRAY_SIZE, Mh_active=FINE_MH_ACTIVE, qv=qv, qh_equiv=qh_a),
            build_wbf_tx_x2_subarray(TX_VERTICAL_SIZE, HORIZONTAL_ARRAY_SIZE, Mh_active=FINE_MH_ACTIVE, qv=qv, qh_equiv=qh_b),
        ]

    if cfg.scheme == "array_refined_fine":
        # Backward-compatible original behavior: selected fine beam qh_a.
        return [build_wbf_tx_x2_subarray(TX_VERTICAL_SIZE, HORIZONTAL_ARRAY_SIZE, Mh_active=FINE_MH_ACTIVE, qv=qv, qh_equiv=qh_a)]
    ...
    if cfg.scheme == "array_comb_re_dmrs_same_center":
        ...
        return [
            build_wbf_tx_x2_subarray(TX_VERTICAL_SIZE, HORIZONTAL_ARRAY_SIZE, Mh_active=FINE_MH_ACTIVE, qv=qv, qh_equiv=qh_center),
            build_wbf_tx_x2_subarray(TX_VERTICAL_SIZE, HORIZONTAL_ARRAY_SIZE, Mh_active=FINE_MH_ACTIVE, qv=qv, qh_equiv=qh_center),
        ]
    ...
    if cfg.scheme == "precoding_comb_re_dmrs":
        ...
        Wbf = build_wbf_tx_x2_upa(qv1=qv, qh1=qh_center, qv2=qv, qh2=qh_center)
        return [Wbf, Wbf]

    if cfg.scheme == "precoding_cyclic":
        if cfg.precoding_basis == "polarization":
            return [build_wbf_tx_x2_upa(qv1=qv, qh1=qh_center, qv2=qv, qh2=qh_center)]
        if cfg.precoding_basis == "spatial_adjacent":
            return [build_wbf_tx_x2_upa(qv1=qv, qh1=qh_a, qv2=qv, qh2=qh_b)]
...
def build_precoded_tx_grids_unified(
    ...
    if Wbf_list is None or len(Wbf_list) == 0:
        Wbf_list = [build_wbf_tx_x2_subarray()]
    ...
    for g, (rb0, rb1) in enumerate(iter_prg_ranges(nrb_sib, prg_size_rb)):
        ...
        if num_wbf == 1:
            ...
            # PRG-level state alternation for two-state array/FDM schemes.
            # With PRG=24RB over 48RB: A, B.
            # With PRG=8RB over 48RB : A, B, A, B, A, B.
            wbf_idx = g % num_wbf
...
def cdl_impulse_response_mimo_full(
    fs_hz,
    ds_ns=300,
    fc_GHz=7,
    seed=None,
    ...
    PBCH reference configuration:
      - cdl_type='CDLC'
      - Tx antenna [1, 1, 6, 4, 2] => 96 Tx ports
      - Rx antenna [1, 1, 1, 2, 2] => 4 Rx ports
      - delay_spread = 300 ns
      - fc = 7 GHz
      - ue_speed = 120 km/h
      - angle_gap = angle_gap_deg, power_gap = 0.0
    ...
    tx_ant = [1, 1, TX_VERTICAL_SIZE, HORIZONTAL_ARRAY_SIZE, NUM_POLARIZATIONS]  # dynamic: 48T/96T/192T
    rx_ant = [1, 1, 1, 2, 2]                                                     # -> 4R
    # angle_gap_deg = 0
    ...
    # scan_qh_gain_for_channel(H_mimo, qv=0.0)
    return H_mimo

def scan_qh_gain_for_channel(H_mimo, qv=0.0):
    out = []
    for qh in np.arange(0, HORIZONTAL_ARRAY_SIZE, 0.25):
        Wbf = build_wbf_tx_x2_subarray(
            Mv=TX_VERTICAL_SIZE,
            Mh_total=HORIZONTAL_ARRAY_SIZE,
            Mh_active=COARSE_MH_ACTIVE,
            qv=qv,
            qh_equiv=qh,
        )
        w_eff = Wbf @ bb_precoder_traditional()
        w_eff = w_eff / np.linalg.norm(w_eff)

        h_eff = np.einsum("rtl,t->rl", H_mimo, w_eff)
        gain = np.sum(np.abs(h_eff) ** 2)
        out.append((qh, 10 * np.log10(gain + 1e-15)))

    out = sorted(out, key=lambda x: x[1], reverse=True)
    print("[QH_SCAN] best:", out[:8])
    return out
...
def apply_cdl_mimo_tx(
    ...
    H_mimo = cdl_impulse_response_mimo_full(
        fs_hz,
        ds_ns,
        fc_hz / 1e9,
        seed,
...
def standard_tdl_impulse_response_mimo_full(
    fs_hz,
    ds_ns=300.0,
    fc_hz=7e9,
    speed_kmh=3.0,
...
def tdl_c_impulse_response(
    ...
    """
    H = standard_tdl_impulse_response_mimo_full(
        fs_hz=fs_hz,
        ds_ns=ds_ns,
        fc_hz=fc_hz,
        speed_kmh=speed_kmh,
...
def apply_tdl_mimo_tx(
    ...
    """
    H_mimo = standard_tdl_impulse_response_mimo_full(
        fs_hz=fs_hz,
        ds_ns=ds_ns,
        fc_hz=fc_hz,
        speed_kmh=speed_kmh,
...
def nrSymbolDemodulate_qpsk_persym(input_sym, nVar_input, re_idx, K):
    ...
    xr, xi = np.real(input_sym), np.imag(input_sym)
    llr = np.empty(2 * len(input_sym), dtype=np.float64)
    llr[0::2] = xr / np.maximum(N0_re, 1e-12)
    llr[1::2] = xi / np.maximum(N0_re, 1e-12)
...
def scan_coarse_ssb_gain_from_H(H_mimo):
    results = []
    for v_idx in range(N_COARSE_V):
        for h_idx in range(N_COARSE_H):
            qv = v_idx * COARSE_QV_STEP
            qh = h_idx * COARSE_QH_STEP
            Wbf = build_wbf_tx_x2_subarray(
                Mv=TX_VERTICAL_SIZE,
                Mh_total=HORIZONTAL_ARRAY_SIZE,
                Mh_active=COARSE_MH_ACTIVE,
            ...
            gain = np.sum(np.abs(h_eff) ** 2)
            ssb_idx = v_idx * N_COARSE_H + h_idx
            results.append((gain, ssb_idx, qv, qh))
...
def _rx_round_get_llr(
    ...
    if channel_model.upper() == "CDL":
        ch_out, H_mimo = apply_cdl_mimo_tx(
            tx_waves,
            fs,
            seed=chan_seed,
            ds_ns=300,
        ...
        # result = scan_coarse_ssb_gain_from_H(H_mimo)
        # print(result)
    ...
    rx_llrs = []
    rx_evms = []
    tx_sym_ref = nrExtractResources(sib_idx, sib_grid)
    ...
    for r in range(rxGrids.shape[0]):
        if ce_mode == "fullband":
            ...
            H_r = rxGrids_clean[r] / sib_grid
        else:
            raise ValueError(f"Unsupported ce_mode: {ce_mode}")


        # tx_sym_ref = nrExtractResources(sib_idx, sib_grid)
        # h_data = nrExtractResources(sib_idx, H_r)
        # y_data = nrExtractResources(sib_idx, rxGrids[r])
        ...
        # DEBUG_CE_EVM = True
        # if DEBUG_CE_EVM and chan_seed == 222607276:
        #     x_data_all = tx_sym_ref
        #     data_eq_all = sib_eq_r
        #     alpha = np.vdot(x_data_all, data_eq_all) / (np.vdot(x_data_all, x_data_all) + 1e-12)
        #     corr = np.abs(np.vdot(x_data_all, data_eq_all)) / (
        #             np.linalg.norm(x_data_all) * np.linalg.norm(data_eq_all) + 1e-12
        #     )
            #
            # print(
            #     f"[DATA_ALPHA] seed={chan_seed}, rx={r}, "
            #     f"alpha_abs={np.abs(alpha):.3f}, "
            #     f"alpha_phase_deg={np.rad2deg(np.angle(alpha)):.2f}, "
            #     f"corr={corr:.3f}"
            # )
        # if chan_seed == 222607276 and abs(snr_db - 40.0) < 1e-6:
        #     x_data_all = nrExtractResources(sib_idx, sib_grid)
        #     data_eq_all = sib_eq_r
        #
        #     alpha = np.vdot(x_data_all, data_eq_all) / (np.vdot(x_data_all, x_data_all) + 1e-12)
        #     corr = np.abs(np.vdot(x_data_all, data_eq_all)) / (
        ...
        #     )
        #
        #     evm = np.mean(np.abs(data_eq_all - x_data_all) ** 2) / np.mean(np.abs(x_data_all) ** 2)
        #
        #     print(
        #         f"[DATA_ALPHA] seed={chan_seed}, snr={snr_db}, rx={r}, "
        #         f"alpha_abs={np.abs(alpha):.3f}, "
        #         f"alpha_phase_deg={np.rad2deg(np.angle(alpha)):.2f}, "
        #         f"corr={corr:.3f}, "
        #         f"evm_dB={10 * np.log10(evm + 1e-15):.2f}"
        #     )
        llr_sum_rx += nrSymbolDemodulate_qpsk_persym(sib_eq_r, nVar_post_r, sib_idx, nrb_sib * 12)
    # print(f"[RX_SELECT] best_rx={best_r}, best_evm_dB={10 * np.log10(rx_evms[best_r] + 1e-15):.2f}")
    return llr_sum_rx * (1 - 2 * meta["scr"].astype(int))
...
def one_shot_sib_min_combined(
    ...
    td_win_len_ratio = scheme_cfg.td_win_len_ratio
    ...
    Wbf_list = make_wbf_list_for_scheme(scheme_cfg, qv, qh_a, qh_b, qh_center=qh_center)
    llr_sum = None
    ...
    # if rng_seed == 222607276:
    #     print(
    #         f"[TRIAL_RESULT] snr={snr_db}, seed={rng_seed}, tb_err={tb_err}"
    #     )
    # print(
    #     f"[TRIAL_RESULT] snr={snr_db}, "
    #     f"best_rx={best_rx}, "
    #     f"tb_err={tb_err}, "
    #     f"best_evm_dB={best_evm_dB}, "
    #     f"seed={rng_seed}"
    # )
    return (tb_err == 0), {}
...
def deterministic_angle_gap_from_trial_seed(trial_seed, angle_gap_max_deg, enable=True):
    """
    Generate one CDL AOD rotation angle for a trial in a scheme-invariant way.

    The angle depends only on trial_seed and angle_gap_max_deg. Since every
    scheme receives the same trial_seed sequence, every scheme experiences the
    same angle_gap for the same SNR/target/trial.
    """
    if (not enable) or angle_gap_max_deg is None or float(angle_gap_max_deg) <= 0.0:
        return 0.0
    seed32 = (int(trial_seed) ^ 0x5DEECE66D) & 0xFFFFFFFF
    rng_angle = np.random.RandomState(seed32)
    return float(rng_angle.uniform(-float(angle_gap_max_deg), float(angle_gap_max_deg)))


def run_single_trial_global(args):
    snr_db, target_mapping, trial_seed, angle_gap_deg, static_cfg, scheme_dict = args

    # Required for Windows/spawned workers: globals are initialized at import
    # time, so each task must reconfigure the beam grid before building Wbf/H.
    configure_beam_grid(static_cfg.get("beam_config_total", CURRENT_BEAM_CONFIG_TOTAL))
...
def simulate_bler_compare(
    ...
    enable_random_angle_gap=ENABLE_RANDOM_ANGLE_GAP_BY_DEFAULT,
    angle_gap_max_deg=DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG,
):
    bler_avg_list = []
    ...
    static_cfg = {
        ...
        "enable_random_angle_gap": bool(enable_random_angle_gap),
        "angle_gap_max_deg": float(angle_gap_max_deg) if angle_gap_max_deg is not None else 0.0,
        "aod_base_deg": float(AOD_BASE_DEG),
        "pass_absolute_aod_to_channel": bool(PASS_ABSOLUTE_AOD_TO_CHANNEL),
        "beam_config_total": int(CURRENT_BEAM_CONFIG_TOTAL),
    }
        ...
        for snr_db in snr_db_list:
            ...
            for mapping in target_mappings:
                ...
                for _ in range(n_trials_per_target):
                    ...
                    angle_offset_deg = deterministic_angle_gap_from_trial_seed(
                        trial_seed,
                        static_cfg["angle_gap_max_deg"],
                        enable=static_cfg["enable_random_angle_gap"] and channel_model.upper() == "CDL",
                    )
                    # Compatibility with the user's current CDL setup:
                    # - PASS_ABSOLUTE_AOD_TO_CHANNEL=False: channel_initialize already contains
                    #   the positive AoD base, so pass only the random offset, matching the old script.
                    # - PASS_ABSOLUTE_AOD_TO_CHANNEL=True: this script supplies base+offset.
                    if static_cfg.get("pass_absolute_aod_to_channel", False):
                        angle_gap_deg = static_cfg["aod_base_deg"] + angle_offset_deg
                    else:
                        angle_gap_deg = angle_offset_deg
                    tasks.append((snr_db, mapping, trial_seed, angle_gap_deg, static_cfg, scheme_dict))
...
# =============================================================================
# Main demo: NR baseline 24RB CE under 8/16/32 beams
# =============================================================================
if __name__ == "__main__":
    ...
    # Use 48RB SIB1 PDSCH bandwidth and 24RB channel-estimation bundle.
    nrb_sib = 48
    fs = 61.44e6
    ...
    if channel_model.upper() == "TDL":
        ...
        snr_points = [-19, -17, -15, -13, -11, -9, -7, -5, -3, -1]

    RUN_ALL_SSB_SECTORS = False
    BEAM_CONFIG_TOTAL_LIST = [8, 16, 32]

    # Only test NR baseline first: legacy DMRS + 24RB CE.
    # All cases use the same 16H physical aperture; only active Np/codebook density changes.
    curves: Dict[str, np.ndarray] = {}
    configs_for_csv = []

    out_csv = "sib1_nr_baseline_common16h_8_16_32beam_24ce_bler_no_shift_idealH.csv"
    out_png = "sib1_nr_baseline_common16h_8_16_32beam_24ce_bler_no_shift_idealH.png"

    for total_beams in BEAM_CONFIG_TOTAL_LIST:
        configure_beam_grid(total_beams)
        center_ssb_idx = default_center_ssb_index_for_aod(AOD_BASE_DEG)
        target_mappings = make_target_mappings_8ssb_16fine(
            run_all_ssb=RUN_ALL_SSB_SECTORS,
            center_ssb_idx=center_ssb_idx,
        )

        print("\n" + "=" * 80)
        print(f"[Beam config] total={CURRENT_BEAM_CONFIG_TOTAL}, grid={N_COARSE_V}Vx{N_COARSE_H}H, "
              f"TX array=(M,N,P)=({TX_VERTICAL_SIZE},{HORIZONTAL_ARRAY_SIZE},{NUM_POLARIZATIONS}), "
              f"active Np={COARSE_MH_ACTIVE}, TX_PORTS={TX_PORTS}, "
              f"COARSE_QH_STEP={COARSE_QH_STEP}, HPBW≈{COARSE_HPBW_DEG:.3f} deg")
        if PASS_ABSOLUTE_AOD_TO_CHANNEL:
            print(f"[CDL AoD] script passes base+offset: base={AOD_BASE_DEG:.3f} deg, "
                  f"offset ~ Uniform(-{DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG:.3f}, +{DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG:.3f}) deg, "
                  f"angle_gap argument range=({AOD_BASE_DEG - DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG:.3f}, "
                  f"{AOD_BASE_DEG + DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG:.3f}) deg")
        else:
            print(f"[CDL AoD] script passes offset only, matching old script: "
                  f"angle_gap ~ Uniform(-{DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG:.3f}, +{DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG:.3f}) deg; "
                  f"AOD_BASE_DEG={AOD_BASE_DEG:.3f} deg is used only for target beam selection")
        for m in target_mappings:
            ...
            print(f"[Target] {m['name']} | qv={qv_dbg}, qh_center={qh_c_dbg}, "
                  ...
                  f"center_ssb_idx={center_ssb_idx}")

        cfg = SchemeConfig(
            label=f"NR baseline: {CURRENT_BEAM_CONFIG_TOTAL} beams ({N_COARSE_V}Vx{N_COARSE_H}H), 24RB CE",
            family="array_based",
        ...
        configs_for_csv.append((total_beams, cfg))
    ...
    with open(out_csv, "w", newline="") as f:
        ...
        writer.writerow([
            ...
            "AOD_Base_Deg",
            "AOD_Random_Span_Deg",
            "Pass_Absolute_AOD_To_Channel",
            "SNR(dB)",
            "BLER",
        ])
        for total_beams, cfg in configs_for_csv:
            configure_beam_grid(total_beams)
            for snr, bler_val in zip(snr_points, curves[cfg.label]):
                writer.writerow([
                    cfg.label,
                    CURRENT_BEAM_CONFIG_TOTAL,
                    N_COARSE_V,
                    N_COARSE_H,
                    TX_VERTICAL_SIZE,
                    HORIZONTAL_ARRAY_SIZE,
                    NUM_POLARIZATIONS,
                    1,
                    COARSE_MH_ACTIVE,
                    TX_DH_LAMBDA,
                    ...
                    cdl_type,
                    AOD_BASE_DEG,
                    DEFAULT_RANDOM_ANGLE_GAP_MAX_DEG,
                    PASS_ABSOLUTE_AOD_TO_CHANNEL,
                    snr,
                    bler_val,
                ])
    ...
    plt.title(f"SIB1 NR baseline 24RB CE: 8/16/32 beams ({channel_model}/{cdl_type})")
